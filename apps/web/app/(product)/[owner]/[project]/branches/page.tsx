...
import { getBranches, getMe, getProject, getReleases } from '../../../../../lib/product';
...
export default async function Branches({ params }: { params: Promise<ProjectParams> }) {
  ...
  const [branches, releases, me] = await Promise.all([getBranches(project.id), getReleases(project.id), getMe()]);
