...
| 10. Paid plans | Yes | 2026-09-25: code shipped (storage limits, Checkout with Managed Payments, portal, webhook). Tax code `txcd_10103000`, prices before tax. Sandbox `acct_1UJdx3EuhsFYXEuz` ("GigaCAD sandbox") set up with `stripe:setup`: 4 products, 8 prices, portal `bpc_1UJeSuEuhsFYXEuz5Ls1tFKD`, webhook to `api.gigacad.site`. Keys and webhook secret are in the ignored `apps/api/.env.stripe`. A test Checkout (Maker monthly, 4242, Seattle address) charged $6.00 + $0.63 WA sales tax (`standard_rated`), sold through Link; test subscription canceled. Railway has the sandbox `STRIPE_*` variables. App test on production with `tararajoshua+billing-check`: `/billing/checkout` → Stripe → back to Plan and storage showing Maker; switching to Builder yearly, cancel at period end, and cancel all synced by webhook (ending on Free, 5 GB); the portal opens. `NEXT_PUBLIC_BILLING_ENABLED` stays `false` while on sandbox keys, so test cards can't buy plans from the pricing page. Live account `acct_1UJdwnROVPYVDZkN` (charges and payouts enabled): `stripe:setup` created the live catalog, portal `bpc_1UJhoTROVPYVDZkNL9AVAnR6`, and webhook; a live Checkout session confirmed Managed Payments with Stripe holding tax liability. The live webhook secret is `STRIPE_WEBHOOK_SECRET` in `apps/api/.env.stripe`. Sandbox billing records were cleared from production. Railway runs the live key and webhook secret (a signed live event is accepted; the sandbox secret is rejected; the API returns `cs_live_` Checkout URLs). `NEXT_PUBLIC_BILLING_ENABLED=true` and the web app is redeployed, so paid plans are on sale at `/pricing` |
